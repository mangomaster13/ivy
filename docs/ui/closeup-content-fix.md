...
- Dictionary: the open-book background, printed overlay and normalized ink share `DictionaryCamera.artworkFrame`. The background covers content; the writing camera fits beside the shared right-hand action rail and above the bottom feedback line (superseding the initial 88 pt bottom control strip). No stroke coordinates or save fields change. The existing transparent English lyric card rests over a camera view of bare stall timber (320×160 source bounds 125,95,64,32), clear of the pen stand and table edge.
- Gelato note: the 512×341 original is proportionally filled into 2:1. The visible source height is 256 pixels (y=42.5–298.5); the complete paper and leaf remain inside this region. The transparent order paper uses bare tasting-counter timber (70,110,64,32), clear of the saucer. Both keep their original lettering. The word menu now retains its complete 2:1 scene on narrow stages instead of putting a cropped rectangle beside controls on Night; Enter/Taste now use the shared 80 pt right-hand action rail; the idle text input retains 92 pt for readable input.
...
## Follow-up: right-hand actions

The user requested a common action location that gives the Dictionary canvas more space. `PuzzleActionLayout` now defines the trailing rail and main interaction rectangle; `DictionaryCamera` fits the same saved ink into that rectangle without changing normalized strokes. The regression check has been updated for rail/ink separation and button-stack bounds, but was not rerun for this ordinary layout revision. The earlier pass above applies only to the preceding closeup fix. No build, device launch or screenshot pass was performed for this follow-up.
