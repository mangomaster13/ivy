...
6. Only after successful stamping, take the card into Tools. Use the cabin door to return to the exit platform. Select the card and use the postbox slot; posting consumes it, saves the original `ferris` collectible and shows Element. The red taxi parked on the forecourt road then leads to Taxi. Revisit keeps the sent card available as a memory and does not require a consumed tool.
