...
- Approved reference: original generated `exec-cc9d2636-137d-4c1e-a4f7-efa3fe286939.png`; production geometry revision: `exec-56196461-c005-4b26-871a-ba6455b09231.png`.
- Reference canvas: 1774 × 887, strict overhead tabletop. Runtime artwork and interactions share the same 2:1 fitted stage.
- Owning surface: wood tabletop for loose tiles; teal recessed tray for assembled tiles. Revised recess approximately x=202...820, y=126...734. A centered 608-unit square working area has an 18-unit inset: board x=225, y=144, width=height=572. The painted side edges leave about 23 units versus 18 above/below, replacing the previous 30-unit side margins and nearly zero vertical margins without stretching the square image.
- Nine square tiles use the existing `city-jigsaw-master` in nine exact crops, preserving the finished collectible. They have 572/3-unit edges and no alpha padding. Each loose position and angle is defined in `CityGridLayout`; index order is back-to-front, selected tile rises above the pile, dragged tile above all others.
...
3. Padding revision: correct only the tray toward a square inner opening, preserving the tabletop, ivy, overhead camera and style. Requested equal clearance around a square puzzle; actual painted bounds were inspected and runtime placement adapted to the resulting artwork, rather than assuming generated coordinates matched the prompt.
