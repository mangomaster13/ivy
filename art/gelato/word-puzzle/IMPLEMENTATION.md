...
The input reuses the approved `ivy-input-v2` asset from the ongoing keyboard work, with its existing native font and field behavior. Only that asset and the necessary root focus preference are included in this feature's checkpoint; other scenes' keyboard conversion remains untouched in the working tree. Input focus hides the footer and repositions the same text field within the keyboard-safe content. The full layout requires 528×264 content; a paper camera crop with a separate action column supports 480×230. Device fit is not yet playtested.
